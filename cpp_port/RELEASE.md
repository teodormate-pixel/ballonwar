...
Dependinte: **GLFW**, **GLEW**, **OpenSSL** (si un compilator C++17).

```bash
# Linux (Ubuntu/Debian)
sudo apt install cmake ninja-build build-essential libglfw3-dev libglew-dev \
...
# macOS (Homebrew)
brew install cmake glfw glew openssl@3 python
cmake -S cpp_port -B build -DCMAKE_BUILD_TYPE=Release \
...
Workflow-ul `.github/workflows/release.yml` construieste pe
`ubuntu-latest`, `macos-latest` (arm64) si `windows-latest`
(biblioteci statice prin vcpkg) si publica cate un ZIP pentru fiecare
platforma.
...
BalloonWar-<versiune>-<platforma>/
  ...
  lib/                      # GLFW/GLEW (doar cand nu sunt statice)
